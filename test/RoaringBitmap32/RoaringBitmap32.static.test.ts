...
describe("RoaringBitmap32 static", () => {
  ...
  describe("static from", () => {
    ...
    it("creates a bitmap from another bitmap", () => {
      ...
      expect(bitmap1 !== bitmap2).toBe(true);
  ...
  describe("addOffset", () => {
    it("returns an empty bitmap if the input bitmap is empty", () => {
      ...
      expect(offsetted).not.eq(input);
      expect(offsetted.toArray()).toEqual([]);
      expect(offsetted.isEmpty).toBe(true);
    });

    it("increment values", () => {
      const input = new RoaringBitmap32([1, 2, 3, 100]);
      const offsetted = RoaringBitmap32.addOffset(input, 10);
      expect(offsetted.toArray()).toEqual([11, 12, 13, 110]);
      expect(offsetted).not.eq(input);
    });
    ...
    it("decrement values", () => {
      ...
      const offsetted = RoaringBitmap32.addOffset(input, -10);
      expect(offsetted.toArray()).toEqual([1, 2, 3, 1090]);
      expect(offsetted).not.eq(input);
    });

    it("accepts out of range values and NaN", () => {
      const input = new RoaringBitmap32([1, 2, 30, 40, 0xffffffff]);
      expect(RoaringBitmap32.addOffset(input, -Infinity).toArray()).toEqual([]);
      expect(RoaringBitmap32.addOffset(input, Infinity).toArray()).toEqual([]);
      expect(RoaringBitmap32.addOffset(input, -0xffffffff - 1).toArray()).toEqual([]);
      expect(RoaringBitmap32.addOffset(input, 0xffffffff).toArray()).toEqual([]);
      expect(RoaringBitmap32.addOffset(input, -0xffffffff).toArray()).toEqual([0]);
      expect(RoaringBitmap32.addOffset(input, 0xfffffffe).toArray()).toEqual([4294967295]);
      expect(RoaringBitmap32.addOffset(input, -35).toArray()).toEqual([5, 4294967260]);
      expect(RoaringBitmap32.addOffset(input, 35).toArray()).toEqual([36, 37, 65, 75]);
    });
  });
  ...
  describe("of", () => {
    ...
    it("creates a bitmap with multiple values", () => {
      ...
      expect(bitmap.isEmpty).eq(false);
      expect(bitmap.isFrozen).eq(false);
      expect(bitmap.toArray()).toEqual([1, 2, 3, 0xffff]);
    });
    ...
    it("handles non-number values", () => {
      ...
      expect(bitmap.isEmpty).eq(false);
      expect(bitmap.isFrozen).eq(false);
      expect(bitmap.toArray()).toEqual([1, 2, 3, 4, 123, 0xffff, 0xffffffff]);
    });
  });
});
