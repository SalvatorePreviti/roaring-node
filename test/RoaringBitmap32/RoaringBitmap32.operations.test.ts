...
describe("RoaringBitmap32 operations", () => {
  ...
  describe("add, remove with multiple arguments", () => {
    it("can add multiple values at once", () => {
      ...
      expect(Array.from(bitmap)).toEqual([1, 2, 3, 4, 5, 7, 8, 9, 10, 12, 60, 199]);
    });

    it("can add multiple values at once 1", () => {
      const bitmap = new RoaringBitmap32();
      ...
      expect(Array.from(bitmap)).toEqual([1, 2, 3, 4, 5, 7, 8, 9, 10, 12, 60, 199, 1000, 0xffffffff]);
    });
    ...
    it("can remove multiple values at once", () => {
      ...
      expect(Array.from(bitmap)).toEqual([2]);
      expect(bitmap.remove("1", 2, 3, 4)).eq(true);
      expect(Array.from(bitmap)).toEqual([]);
  ...
  describe("remove", () => {
    it("remove does nothing on an empty bitmap", () => {
      const bitmap = new RoaringBitmap32();
      expect(bitmap.remove(19)).toBe(false);
      expect(bitmap.size).eq(0);
    });

    it("remove one item", () => {
      const bitmap = new RoaringBitmap32([1, 5, 6]);
      expect(bitmap.remove(5)).toBe(true);
      expect(bitmap.toArray()).toEqual([1, 6]);
      expect(bitmap.remove(5)).toBe(false);
      expect(bitmap.toArray()).toEqual([1, 6]);
    });
  });
  ...
  describe("removeMany", () => {
    ...
    it("removeMany removes multiple items (Uint32Array) 1", () => {
      const bitmap = new RoaringBitmap32([1, 2, 123, 3, 4, 5, 6]);
      bitmap.removeMany(new Uint32Array([123, 5, 6]));
      expect(bitmap.toArray()).toEqual([1, 2, 3, 4]);
    });
